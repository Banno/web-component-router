...
class RouteTreeNode {
  ...
  async activate(previousRouteId, context) {
    ...
    console.log('exits?', exitNodes.length)
    ...
    for (let entryIndex = 0; entryIndex < entryNodes.length; entryIndex++) {
      ...
      if (currentEntryNode.getValue().element) {
        console.log('routeEnter test', currentEntryNode.getKey(), currentEntryNode.getValue().element.routeEnter);
        const routingElem = /** @type {!BasicRoutingInterface} */ (
            /** @type {?} */ (currentEntryNode.getValue().element)
        );
        let shouldContinue = await loadRouteNode(currentEntryNode, nextEntryNode, routeId, context);
        console.log('shouldContinue after loadRouteNode: ', shouldContinue !== false, routingElem);
        if (routingElem.routeEnter) {
          console.log('calling routeEnter for: ', currentEntryNode.getKey());
