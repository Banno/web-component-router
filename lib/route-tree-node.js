...
class RouteTreeNode {
  ...
  async activate(previousRouteId, context) {
    ...
    const entryNodes = [/** @type {!RouteTreeNode} */ (this)].concat(
      this.getAncestors(),
    );
    entryNodes.reverse();
    let exitNodes = [];
    ...
    if (previousRouteId !== undefined) {
      ...
      const previousRoutePath = [previousRouteNode].concat(
    ...
    for (let exitIndex = 0; exitIndex < exitNodes.length; exitIndex++) {
      ...
      if (value) {
        ...
        if (!routingElem.routeExit) {
          throw new Error(
            `Element '${currentExitNode.getValue().tagName}' does not implement routeExit`,
          );
        }
        await routingElem.routeExit(
          currentExitNode,
          nextExitNode,
          routeId,
          context,
    ...
    for (let entryIndex = 0; entryIndex < entryNodes.length; entryIndex++) {
      ...
      if (currentEntryNode.getValue().element) {
        ...
        if (!routingElem.routeEnter) {
          throw new Error(
            `Element '${currentEntryNode.getValue().tagName}' does not implement routeEnter`,
          );
        }
        const shouldContinue = await routingElem.routeEnter(
          currentEntryNode,
          nextEntryNode,
          routeId,
          context,
