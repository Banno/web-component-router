...
class RouteTreeNode {
  ...
  async activate(previousRouteId, context) {
    ...
    for (let exitIndex = 0; exitIndex < exitNodes.length; exitIndex++) {
      ...
      if (value) {
        await this.routeExit(currentExitNode, nextExitNode, routeId, context);
    ...
    for (let entryIndex = 0; entryIndex < entryNodes.length; entryIndex++) {
      ...
      const routingElement = currentEntryNode.getValue().element;
      if (routingElement) {
        let shouldContinue;
        if (routingElement.routeEnter) {
          shouldContinue = await routingElement.routeEnter(currentEntryNode, nextEntryNode, routeId, context);
        } else {
          shouldContinue = await this.routeEnter(currentEntryNode, nextEntryNode, routeId, context);
        }
