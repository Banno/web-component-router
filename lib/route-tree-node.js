  ...
  async activate(previousRouteId, context) {
    ...
    for (let entryIndex = 0; entryIndex < entryNodes.length; entryIndex++) {
      ...
      if (currentEntryNode.getValue().element) {
        ...
        const shouldContinue = await loadRouteNode(currentEntryNode, nextEntryNode, routeId, context);
        if (shouldContinue === false) {
          break;
        }
        if (routingElem.routeEnter) {
          await routingElem.routeEnter(currentEntryNode, nextEntryNode, routeId, context);
        }
      }
    }
  }
