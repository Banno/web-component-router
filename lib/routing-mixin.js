...
/**
 * @param {CustomElementConstructor} Superclass
 * @polymer
 * @mixinFunction
 */
function routingMixin(Superclass) {
  ...
  class BasicRouting extends Superclass {
    ...
    async routeEnter(currentNode, nextNodeIfExists, routeId, context) {
      ...
      const currentElement = /** @type {!Element} */ (currentNode.getValue()
        .element);
      ...
      if (nextNodeIfExists) {
        ...
        /** @type {Element} */
        let nextElement =
          nextNodeData.element ||
          thisElem.querySelector(nextNodeData.tagName.toLowerCase());
        ...
        if (!nextElement || nextElement.parentNode !== currentElement) {
          if (nextNodeData.tagName.indexOf("-") > 0) {
            let Elem =
              customElements &&
              customElements.get(nextNodeData.tagName.toLowerCase());
            if (!Elem) {
              ...
              await customElements.whenDefined(
                nextNodeData.tagName.toLowerCase()
              );
