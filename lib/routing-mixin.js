...
function routingMixin(Superclass) {
  ...
  class BasicRouting extends Superclass {
    ...
    async routeEnter(currentNode, nextNodeIfExists, routeId, context) {
      ...
      console.log('routeEnter', currentNode, nextNodeIfExists, routeId, context);
      ...
      if (nextNodeIfExists) {
        ...
        if (!nextElement || nextElement.parentNode !== currentElement) {
          if (nextNodeData.tagName.indexOf('-') > 0) {
            ...
            console.log('here?', nextNodeData.tagName);
            if (!Elem) {
              // if the nextNodeData has an import function, use it to load the element
              console.log('importFunction', nextNodeData);
              if (nextNodeData.importFunction) {
