...
class SectionBRoute extends routingMixin(LitElement) {
  ...
  render() {
    return html`
      ...
      <p>
        This route has two child routes (B1 and B2). Each has a unique param
        which the router gets from the path and sets on the component.
      </p>
      <p>
        This parent component is loaded for each sub route, but not reloaded
        when routing between children, so shared actions can be taken here for
        performance.
      </p>
      <ul>
