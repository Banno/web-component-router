...
describe("Router", () => {
  // Add this block to reset the singleton before each constructor test
  beforeEach(() => {
    Router.instance_ = null;
  });

  afterAll(() => {
    // It's good practice to reset again after these tests are done
    Router.instance_ = null;
  });
  ...
  it(".start should register routes and start routing", () => {
    ...
    expect(router.page.callbacks.length).toBe(
      5 + initialCallbackLength + builtinCallbackLength,
    );
    ...
    it("should create a routeTree with the correct properties", () => {
      ...
      subRoutes.forEach((route, index) => {
        ...
        if (testSubRouteData[index].params) {
          expect(Object.keys(data.attributes)).toEqual(
            testSubRouteData[index].params,
          );
        }
  ...
  describe("url()", () => {
    ...
    it("should replace path parameters with given values", () => {
      expect(
        router.url("/account/:accountId([-a-fA-F0-9]{36})/documents/:docId", {
          accountId: "1234",
          docId: "6789",
        }),
    ...
    it("should allow a trailing slash", () => {
      ...
      expect(
        router.url("/account/:accountId([-a-fA-F0-9]{36})/", {
          accountId: "1234",
        }),
      ).toBe("/account/1234/");
    });
    ...
    it("should combine multiple slashes", () => {
      const url = router.url(
        "/account/:fromAccountId([0-9]+)?/:toAccountId([0-9]+)?/",
      );
  ...
  describe("go()", () => {
    beforeEach(() => {
      vi.spyOn(
        router.page,
        JSCompiler_renameProperty("show", router.page),
    ...
    it("should replace path parameters with given values", () => {
      ...
      expect(router.page.show).toHaveBeenCalledWith(
        "/account/1234/documents/6789",
      );
    });
  ...
  describe("routeEnter", () => {
    it("should create the next element when it does not exist", async () => {
      ...
      expect(B.getValue().element.tagName.toLowerCase()).toBe(
        testRouteTree.Id.B,
      );
    });
    ...
    it("when reusing an element, it should still update the attributes", async () => {
      ...
      B.getValue().element = bElement;
      aElement.appendChild(bElement);
      await RoutedElement.prototype.routeEnter(
        A,
        B,
        testRouteTree.Id.B,
        context,
  ...
  describe("routeExit", () => {
    ...
    it("should remove the element from the parent node", async () => {
      ...
      expect(
        A.getValue().element.getElementsByTagName(testRouteTree.Id.B).length,
      ).toBe(0);
    });
  });
});
