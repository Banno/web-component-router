...
describe("RouteTreeNode", () => {
  ...
  /** @polymer */
  class RoutedElement extends HTMLElement {
    // eslint-disable-line @banno/ux/custom-element-name
    static get is() {
  ...
  describe("activate function", () => {
    ...
    it("activating a route should call the correct methods", async () => {
      ...
      expect(routePath.join("_")).toBe(
        "B-exit_A-exit_ROOT-enter_D-enter_E-enter",
      );
    });
    ...
    it('returning "false" from the routeEnter method should prevent future methods from being invoked', async () => {
      vi.spyOn(A.getValue().element, "routeEnter").mockImplementation(
        function () {
